...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => { // Add event type
        ...
        try {
            ...
            // Check if data and data.user are not null before proceeding
            if (data?.user) {
                ...
                try {
                    ...
                    // Check for Supabase specific error code PGRST116 (No rows found)
                    if (
                        profileError &&
                        (profileError as any).code === "PGRST116" // Type assertion for code
                    ) {
                        ...
                        // Handle other profile errors if necessary
                        throw profileError;
                    }
                } catch (profileError) {
                    console.error(
                        "Erro ao verificar/criar perfil:",
                        profileError
                    );
            ...
            const errorMessages: { [key: string]: string } = { // Add type for errorMessages
